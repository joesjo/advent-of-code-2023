    ...
    for (const searchLine of searchLines) {
      ...
      for (const partNumber of partNumbers[searchLine]) {
        if (partNumber.positions.some(position => matchingPositions.includes(position))) {
          matchingPartNumbers.push(partNumber.partNumber);
        }
      }
    }
    if (matchingPartNumbers.length === 2) {
      correctPartNumbers.push(parseInt(matchingPartNumbers[0]) * parseInt(matchingPartNumbers[1]));
    }
  }
}
